...
* The index refers to the index number shown in the displayed item list.
* The index *must be a positive integer* `1, 2, 3, ...`
* NUMBER_OF_INGREDIENTSS for an ingredient must follow the INGREDIENT_NAME for that particular ingredient.
****
...
2) The required ingredients to make one unit of `ITEM_NAME` is specified. Refer to <<add-required-ingredient>> for more
information. +
...
==== Check item sales history `[coming in v2.0]`
...
==== Display sales graph `[coming in v2.0]`
...
==== Export sales report`[coming in v2.0]`
...
==== Show history of past stock ups and consumption `[coming in v2.0]`
