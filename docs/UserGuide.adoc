...
=== Recording sales volume of a menu item within a day: `record-sales`

Records the quantity of a menu item sold within the selected day into the day's sales report. +
...
OR +
Displays the sales report for a specified day +
...
=== Deleting a sales record / report for a day: `delete-sales`

Deletes a record in the sales report for a specified day +
Format: `delete-sales DATE ITEM_NAME` / `delete-sales DATE ITEM_INDEX` +
OR +
Deletes the entire sales report for a specified day. +
...
* The index *must be a positive integer* 1, 2, 3, ...
* Alternatively, deletes the record with the specified `ITEM_NAME`.
