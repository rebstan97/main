...
[NOTE]
====
Sales volume will still be recorded even if not all the above conditions are satisfied. However, ingredient list would
 not be updated.
====
...
[NOTE]
====
Deleting a sales record will not undo the effect of <<Auto-ingredient update>> when recording.
====

=== Editing a sales record: `edit-sales` or `es`

Edits the sales record identified by the index. +
Format: `edit-sales ITEM_INDEX [d/DATE] [n/ITEM_NAME] [q/QUANTITY SOLD] [p/ITEM_PRICE]`
...
[NOTE]
====
Editing a sales record will not undo the effect of auto-ingredient update when recording.
====
