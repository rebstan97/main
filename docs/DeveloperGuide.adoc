...
==== Current Implementation
The menu is stored internally as `items`, which is an `UniqueItemList` object that contains a list of `Item` object.
...
==== `add-item` Command
The `add-item` command is facilitated by `AddItemCommand` and `AddItemCommandParser`. `add-item` command takes in
user input for `Name`, `Price` and `Tag`(optional). `AddItemCommandParser` will parse the user input and checks if
the input is valid. If the input is not valid, it will throw a `ParseException`. If the input is valid, it will
constructs an `Item` object and create `AddItemCommand` object.

The `AddItemCommand` will indirectly call the `add(Item toAdd)` method of `UniqueItemList` and the adds the item.
After it adds the item, it will save the current state for undo/redo. 
