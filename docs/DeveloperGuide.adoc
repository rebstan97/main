...
The `AddItemCommandParser` will parse the user input and checks if the input is valid and constructs an `Item` object.
[NOTE]
...
user input for the sorting method(Currently only sort by name or price).
