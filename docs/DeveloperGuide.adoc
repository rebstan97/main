...
The menu management feature extends `MeNuS` with a menu, stored internally as an `items`.
It implements the following operations:

* `UniqueItemList#add(Item toAdd)` -- Adds an item to the list. The item must not already exist in the list.
* `UniqueItemList#setItem(Item target, Item editedItem)` -- Replaces the target item with the editedItem. Target item
 must be in the list and editItem must not be the same as another existing item in the list.
* `UniqueItemList#remove(Item toRemove)` -- Removes the equivalent item from the list. The item must exist in the list.
* `UniqueItemList#sortItemsByName()` -- Sorts the contents of this list by name in alphabetical order.
* `UniqueItemList#sortItemsByPrice()` -- Sorts the contents of this list by price in ascending order.
// end::menu[]
