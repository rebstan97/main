...
// tag::filtermenu[]
=== Filter Menu feature
The filter menu feature works by using the updateFilteredItemList method in the model manager. The filterMenuParser
will parse the user input and create a predicate which is use with the updateFilteredItemList method.

Given below is an example usage scenario and how the filter menu feature works at each step.

Step 1. The user launches the application. User executes the filter menu command with keywords.

Step 2. The filterMenuParser parses the user input into a TagContainsKeywordsPredicate which is a predicate that is
true when an item's set of tags contains any of the keywords, and create a filterMenuCommand.

Step 3. The LogicManager executes the filterMenuCommand which calls the updateFilteredItemList method with the
